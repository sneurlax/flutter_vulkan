//! C FFI exports consumed by Flutter platform plugins.
...
// CString return buffer (valid until next call on same thread).
thread_local! {
...
/// # Safety
/// `val` must point to correctly sized/aligned memory for `uniform_type`.
...
/// # Safety
/// `buffer` must point to at least `width * height * 4` valid bytes.
#[no_mangle]
pub unsafe extern "C" fn createRenderer(buffer: *mut u8, width: i32, height: i32) {
    ...
    // tear down previous renderer (avoid double-lock by inlining)
    {
...
/// # Safety
/// `width` and `height` must be valid, aligned `*mut i32`.
#[no_mangle]
...
/// # Safety
/// `user_data` must remain valid for the renderer's lifetime.
#[no_mangle]
...
/// # Safety
/// `vertex` and `fragment` must be null or valid NUL-terminated C strings.
#[no_mangle]
...
/// # Safety
/// `fragment` must be null or a valid NUL-terminated C string.
#[no_mangle]
...
/// # Safety
/// `name`: valid C string. `val`: correctly sized for `uniform_type`.
#[no_mangle]
...
/// # Safety
/// `name`: valid C string. `val`: correctly sized for the uniform's type.
#[no_mangle]
...
/// # Safety
/// `name`: valid C string. `val`: at least `width * height * 4` bytes of RGBA8.
#[no_mangle]
